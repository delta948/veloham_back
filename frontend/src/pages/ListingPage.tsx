    ...
    <div className="space-y-10">
      <div className="grid gap-8 lg:grid-cols-[1.2fr_.8fr]">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            {listing.images?.slice(1).map((img) => <img key={img.id} className="aspect-video w-full object-cover" src={listingImageUrl(img.image_url)} alt={listing.title} />)}
          </div>
        </div>
        <aside className="panel space-y-5 p-6">
          ...
          <h1 className="text-5xl font-black uppercase">{listing.title}</h1>
          <PriceDropBadge listing={listing} detailed />
          <div className="text-5xl font-black text-acid">{formatSom(listing.price)}</div>
          ...
          <div className="grid gap-2 text-white/80">
            ...
            <Link to={`/profile/${listing.user_id}`} className="text-acid">Продавец: <b>{listing.user?.username}</b> · рейтинг {Number(listing.user?.rating ?? 0).toFixed(1)}</Link>
          </div>
      ...
      {buildRows.length > 0 && (
        <section className="panel p-6">
          <h2 className="text-4xl font-black uppercase">Build Card</h2>
      ...
      {listing.match_preference?.exchange_enabled && (
        <section className="panel p-6">
          <h2 className="text-4xl font-black uppercase">Bike Match</h2>
      ...
      {similar.length > 0 && (
        <section className="space-y-5">
          <h2 className="text-4xl font-black uppercase">Похожие объявления</h2>
