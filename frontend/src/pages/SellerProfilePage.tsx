  ...
  return (
    <div className="space-y-8">
      <section className="panel flex flex-col gap-5 p-6 md:flex-row md:items-center">
        <img className="h-24 w-24 border border-white/15 object-cover" src={seller.avatar_url || 'https://images.unsplash.com/photo-1508214751196-bcfd4ca60f91?q=80&w=400&auto=format&fit=crop'} alt={seller.username} />
        <div>
          <h1 className="text-5xl font-black uppercase">{seller.username}</h1>
          <p className="text-white/60">{seller.city || 'Кыргызстан'} · рейтинг {Number(seller.rating ?? 0).toFixed(1)} · с {new Date(seller.created_at).toLocaleDateString('ru-KG')}</p>
