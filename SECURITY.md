...
- Passwords must contain 12 to 72 characters and are hashed with bcrypt.
...
The backend passes `go test ./...`, `go vet ./...`, and `govulncheck` with no reachable vulnerabilities. The frontend production build passes.
...
- Add PostgreSQL-backed integration tests for authorization boundaries and migrations.
...
- Configure explicit trusted reverse-proxy addresses when deploying behind a load balancer; none are trusted by default.
